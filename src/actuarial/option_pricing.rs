...
pub fn binomial_option_value(
    ...
    let strike_price = if strike_price == 0.0 { 0.001 } else { strike_price };
    let vesting_period = vesting_period.min(maturity);
    ...
    if (vesting_period - maturity).abs() < f64::EPSILON {
        return Ok(vec![black_scholes_call_option_value(
                        share_price, strike_price, maturity, risk_free,
                        div_rate, sigma,),
                    maturity,]);
    }
    ...
    for i in 0..=steps {
        ...
        macaulay_numerator[idx(steps, i)] = intrinsic_value[idx(steps, i)] * maturity;
    }
...
#[xl_func()]
pub fn binomial_option_value_xxxx(
    share_price: f64,
    strike_price: f64,
    maturity: f64,
    vesting_period: f64,
